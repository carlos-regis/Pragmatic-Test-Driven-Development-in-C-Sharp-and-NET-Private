...
public class SlotsServiceTests : IDisposable
{
    ...
    {
        // Arrange
        DateTime appointmentFrom = new(2022, 10, 3, 7, 0, 0, DateTimeKind.Local);
    ...
    public async Task GetAvailableSlotsForEmployee_OneShiftAndNoExistingAppointments_VaryingSlots(
         int shiftDuration, int totalSlots, params string[] expectedTimes)
    {
        // Arrange
        DateTime timeOfBooking = new(2022, 10, 3, 7, 0, 0, DateTimeKind.Local);
        _nowService.Now.Returns(timeOfBooking);
        DateTime shiftFrom = new(2022, 10, 3, 9, 0, 0, DateTimeKind.Local);
        ...
        {
            Assert.Equal(DateTime.Parse(expectedTimes[i], new CultureInfo("en-US")), times[i]);
        }
    }
    ...
    {
        // Arrange
        DateTime timeOfBooking = new(2022, 10, 3, 7, 0, 0, DateTimeKind.Local); // 07:00
        _nowService.Now.Returns(timeOfBooking);
        DateTime shiftFrom = new(2022, 10, 3, 9, 0, 0, DateTimeKind.Local); // 09:00
        DateTime shiftTo = new(2022, 10, 3, 11, 10, 0, DateTimeKind.Local); // 11:10
        DateTime appointmentStart = DateTime.Parse(appointmentStartStr, new CultureInfo("en-US"));
        DateTime appointmentEnd = DateTime.Parse(appointmentEndStr, new CultureInfo("en-US"));
        var context = _contextBuilder
            .WithSingleService(30)
            .WithSingleEmployeeTom()
            .WithSingleShiftForTom(shiftFrom, shiftTo)
        ...
        {
            var time = times[i];
            Assert.Equal(DateTime.Parse(expectedTimes[i], new CultureInfo("en-US")), time);
        }
    }
